...
export default function ActorDetail({ params }) {
  ...
  useEffect(() => {
    const fetchActor = async () => {
      try {
        const response = await fetch(`http://localhost:3001/api/movies/actor/${name}`);
        if (!response.ok) {
  ...
  const actorImage = actor.image
    ? actor.image
    : "/default-actor-image.jpg"; // Ruta a la imagen predeterminada en la carpeta public

  return (
    <div className="flex justify-center items-center h-screen bg-gray-900">
      <div className="flex flex-col items-center bg-gray-800 p-6 rounded-lg shadow-md">
        <img src={actorImage} alt={actor.name} className="w-32 h-32 rounded-full mb-4" />
